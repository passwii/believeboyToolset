...
# 导入应用逻辑
try:
    from apps.toolset.research_analysis import ResearchAnalyzer
    from apps.toolset.excel_formula_remover import process_excel_file_stream
...
def operations_overview():
    """运营总览页面"""
    LogService.log(
        action="访问运营总览", resource="运营总览", log_type="user", level="info"
    )
...
def operations_nav():
    """运营导航页面"""
    LogService.log(
        action="访问运营导航", resource="运营导航", log_type="user", level="info"
    )
...
def shop_nav():
    ...
    # 记录访问店铺导航页面日志
    LogService.log(
        action="访问店铺导航", resource="店铺导航", log_type="user", level="info"
    )
    ...
    return render_template(
        "operations/shop_nav_embed.html",
...
def research_analysis():
    ...
    # 记录访问调研分析页面日志
    LogService.log(
        action="访问调研分析", resource="调研分析", log_type="user", level="info"
    )
...
def upload_research_file():
    ...
    if ResearchAnalyzer is None:
        return jsonify({"success": False, "message": "研究分析模块未正确加载"})
    ...
    try:
        ...
        if not analyzer.run_all_analysis():
            ...
            if os.path.exists(filepath):
                os.remove(filepath)
            return jsonify(
                {"success": False, "message": "数据分析失败，请检查文件格式"}
            )
...
def cleanup_temp_files():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": f"已清理 {cleaned_count} 个临时文件"}
        )
...
def exchange_rate_display():
    """汇率展示页面"""
    LogService.log(
        action="访问汇率展示", resource="汇率展示", log_type="user", level="info"
    )
...
def img_believeboy():
    """图片图床页面"""
    LogService.log(
        action="访问图片图床", resource="图片图床", log_type="user", level="info"
    )
...
def proxy_img_believeboy():
    ...
    try:
        ...
        return (
            ...
            .set_data(content.encode("utf-8"))
            .headers.set_list("Set-Cookie", new_headers)
        )
    except Exception as e:
