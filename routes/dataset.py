...
from apps.dataset.daily_report import daily_report_bp
from apps.dataset.monthly_report import monthly_report_bp
from apps.dataset.product_analysis import product_analysis_bp
...
def daily_report_page():
    # 记录访问日报页面日志
    LogService.log(
        action="访问日报页面", resource="数据集", log_type="user", level="info"
    )
    # 获取项目列表
...
def monthly_report():
    # 记录访问月报页面日志
    LogService.log(
        action="访问月报页面", resource="数据集", log_type="user", level="info"
    )
    # 获取项目列表
    projects_path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
...
def product_analysis():
    # 记录访问产品分析页面日志
    LogService.log(
        action="访问产品分析页面", resource="数据集", log_type="user", level="info"
    )
    # 获取项目列表
    projects_path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
...
def timeline():
    # 记录访问时间线页面日志
    LogService.log(
        action="访问时间线页面", resource="数据集", log_type="user", level="info"
    )
