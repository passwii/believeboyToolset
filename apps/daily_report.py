...
def upload_file():
    ...
    try:
        ...
        if not file_type or file_type not in ['sales_report', 'fba_report', 'ad_report']:
...
def process_daily_report_from_paths(project_name, report_date, sales_report_path, ad_report_path, fba_report_path):
    """从文件路径处理日报"""
...
def daily_report():
    if request.method == 'POST':
        ...
        # 验证文件格式（如果有文件上传）
        if has_files:
            if not (allowed_file(sales_report_file.filename) and allowed_file(ad_report_file.filename) and allowed_file(fba_report_file.filename)):
                flash('文件格式不正确')
                return redirect(url_for('dataset.daily_report_page'))
        ...
        try:
            ...
            if has_paths:
                # 验证文件是否存在
                if not all(os.path.exists(path) for path in [sales_report_path, ad_report_path, fba_report_path]):
                    flash('文件不存在，请重新上传')
                    return redirect(url_for('dataset.daily_report_page'))
                ...
                file_content, filename = process_daily_report(
                    project_name, report_date, sales_report_file, ad_report_file, fba_report_file
                )
