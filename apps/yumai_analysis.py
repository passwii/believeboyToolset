from flask import Blueprint, request, send_file, jsonify, render_template, flash, redirect, url_for
import io
import os
import csv
import openpyxl
...
def process_yumai_data(yumai_report_path, inventory_report_path=None):
    """处理优麦云数据并可选择性地添加库存详情"""
    ...
    ordered_columns = ['SKU', 'ASIN', '币种', '销量', '销售额', '可售', '广告花费', '广告曝光量', '广告点击量', '广告占比']
    ...
    summary = {'销量': 0, '销售额': 0, '广告花费': 0, '广告曝光量': 0, '广告点击量': 0, '可售': 0}
    ...
    for r_idx, row_data in enumerate(data_rows, 2):
        for col_name in source_columns:
            ...
            new_cell = new_ws.cell(row=r_idx, column=new_col_idx, value=row_data[col_name])
    ...
    summary_row_data = {
        'SKU': '汇总', 'ASIN': '', '币种': '',
        '销量': summary['销量'], '销售额': summary['销售额'], '可售': summary['可售'],
        '广告花费': summary['广告花费'], '广告曝光量': summary['广告曝光量'], '广告点击量': summary['广告点击量'],
        '广告占比': ''
    }
    ...
    if inventory_report_path and os.path.exists(inventory_report_path):
        ...
        keep_cols = ['sku', 'asin', 'available', 'inv-age-0-to-90-days', 'inv-age-91-to-180-days', 'inv-age-181-to-270-days', 'inv-age-271-to-365-days', 'inv-age-365-plus-days', 'recommended-action']
        inv_df = inv_df[[col for col in keep_cols if col in inv_df.columns]].copy()
        
        rename_dict = {'sku': 'SKU', 'asin': 'ASIN', 'available': '可售库存', 'inv-age-0-to-90-days': '0-90天', 'inv-age-91-to-180-days': '91-180天', 'inv-age-181-to-270-days': '181-270天', 'inv-age-271-to-365-days': '271-365天', 'inv-age-365-plus-days': '365+天', 'recommended-action': '库存建议'}
        ...
        new_column_order = ['SKU', 'ASIN', '本周销量', '可售库存', '0-90天', '91-180天', '181-270天', '271-365天', '365+天', '库存建议']
        inv_df = inv_df[[col for col in new_column_order if col in inv_df.columns]]

        summary_data = {'SKU': '汇总行', 'ASIN': '', '本周销量': inv_df['本周销量'].sum(), '可售库存': inv_df['可售库存'].sum(), '0-90天': inv_df['0-90天'].sum(), '91-180天': inv_df['91-180天'].sum(), '181-270天': inv_df['181-270天'].sum(), '271-365天': inv_df['271-365天'].sum(), '365+天': inv_df['365+天'].sum(), '库存建议': ''}
...
def upload_file():
    ...
    try:
        ...
        if not file_type or file_type not in ['yumai_report', 'inventory_report']:
        ...
        if file and allowed_file(file.filename):
            ...
            LogService.log(action="上传优麦分析文件", resource="优麦分析", details=f"项目: {project_name}, 文件类型: {file_type}, 文件名: {original_filename}", log_type="user", level="info")
            
            return jsonify({'success': True, 'file_path': file_path, 'filename': new_filename, 'original_filename': original_filename})
        else:
            LogService.log(action="上传优麦分析文件失败", resource="优麦分析", details=f"项目: {project_name}, 文件类型: {file_type}, 错误: 文件类型不支持", log_type="user", level="warning")
            return jsonify({'success': False, 'error': '文件类型不支持'}), 400
            
    except Exception as e:
        LogService.log(action="上传优麦分析文件异常", resource="优麦分析", details=f"项目: {request.form.get('project_name')}, 错误: {str(e)}", log_type="system", level="error")
...
def submit_yumai_analysis():
    ...
    try:
        ...
        if not all([project_name, report_start_date, report_end_date, yumai_report_path]):
        ...
        result_wb = process_yumai_data(yumai_report_path, inventory_report_path)
        ...
        LogService.log(action="生成优麦分析报告", resource="优麦分析", details=f"项目: {project_name}, 日期: {report_date}", log_type="user", level="info")
        ...
        LogService.log(action="生成优-麦分析报告失败", resource="优麦分析", details=f"错误: {str(e)}", log_type="system", level="error")
        flash(f'处理失败: {str(e)}')
        return redirect(url_for('yumai_analysis.yumai_analysis_page'))
    except Exception as e:
        LogService.log(action="生成优麦分析报告异常", resource="优麦分析", details=f"错误: {str(e)}", log_type="system", level="error")
