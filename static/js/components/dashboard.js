...
class Dashboard {
  ...
  bindEvents() {
    // 统计卡片点击事件
    DOM.findAll('.stat-card').forEach(card => {
    ...
    // 刷新按钮事件
    const refreshBtn = DOM.find('.refresh-btn');
    if (refreshBtn) {
  ...
  updateStatCard(type, data) {
    ...
    switch(type) {
      case 'daily':
        cardElement = DOM.find('[data-stat-type="daily"]');
      ...
      case 'monthly':
        cardElement = DOM.find('[data-stat-type="monthly"]');
        valueElement = cardElement?.querySelector('.stat-value');
        subtitleElement = cardElement?.querySelector('.stat-subtitle');
      ...
      case 'product-analysis':
        cardElement = DOM.find('[data-stat-type="product-analysis"]');
        valueElement = cardElement?.querySelector('.stat-value');
        subtitleElement = cardElement?.querySelector('.stat-subtitle');
      ...
      case 'total':
        cardElement = DOM.find('[data-stat-type="total"]');
        valueElement = cardElement?.querySelector('.stat-value');
        subtitleElement = cardElement?.querySelector('.stat-subtitle');
   ...
   */
  renderTrendChart() {
    const canvas = DOM.find('#trend-chart');
   ...
   */
  renderPieChart() {
    const canvas = DOM.find('#pie-chart');
  ...
  renderRecentActivities() {
    const container = DOM.find('#activity-list');
    ...
    container.innerHTML = activities.map(activity => `
      <li class="activity-item">
        ...
        <div class="activity-time">${TimeUtils.timeAgo(activity.timestamp)}</div>
      </li>
    `).join('');
  }
  ...
  renderSystemStatus() {
    ...
    const lastLoginText = status.last_login 
      ? `${status.last_login.username} - ${TimeUtils.timeAgo(status.last_login.timestamp)}`
      : '无记录';
  ...
  updateStatusItem(type, data) {
    const element = DOM.find(`[data-status-type="${type}"]`);
    if (!element) return;
  ...
  navigateToPage(contentType) {
    // 触发对应导航项的点击事件
    const navLink = DOM.find(`[data-content="${contentType}"]`);
    if (navLink) {
      navLink.click();
    }
  }
   ...
   */
  showLoading() {
    const container = DOM.find('.dashboard-container');
    if (container) {
      container.classList.add('loading');
    }

    // 显示活动列表的加载指示器
    const activityList = DOM.find('#activity-list');
    if (activityList) {
...
function initDashboard(options = {}) {
  ...
  globalDashboard = new Dashboard(options);
  return globalDashboard;
}
...
document.addEventListener('DOMContentLoaded', function() {
  // 检查是否在仪表板页面
  const dashboardContainer = DOM.find('.dashboard-container');
  const defaultSection = DOM.find('.default-section');
  
  if (dashboardContainer || (defaultSection && defaultSection.classList.contains('active'))) {
    // 等待内容加载完成后再初始化仪表板
    setTimeout(() => {
